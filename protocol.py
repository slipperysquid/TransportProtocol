...
class Protocol():
    ...
    def conn(self,dest_IP,dest_port):
        ...
        while not(connected):
            ...
            initial_packet = Packet(self.IP,self.port, dest_IP,dest_port ,sequence=0, data=None,ack=0,syn=True).build()
        ...
        return Connection(self.IP,self.port,dest_IP,dest_port,self.socket)
    ...
    def accept(self):
        ...
        while not(connected):
            ...
            if (Packet.validate_packet(packet=data,checksum=checksum) == False):
                ...
                #create synack
                synack=Packet(sender_IP=self.IP,sender_port=self.port, dest_IP=socket.inet_ntoa(sender),dest_port=sender_port ,sequence=0, data=None,ack=1,syn=True,is_ack=True).build()
                #send synack
                ...
                #check for ack bit
                    ...
                    connected = True
        return Connection(self.IP,self.port, socket.inet_ntoa(sender),sender_port,self.socket)